...
public class greatestNrOcurrenciesHeap {
    ...
    public static Heap<Tuple> get(int k, Scanner scanners[]){
        ...
        Heap<Tuple> heap = new Heap<>((x, y) -> Integer.compare(x.counter, y.counter));
    ...
    public static Tuple[] sort(int k, Scanner scanners[]){
        ...
        Heap<Tuple> heap = get(k, scanners);

        //To organize it calls a method in the Heap class that changes what variables it uses to compare
        //instead of comparing the number of occurrences it will now compare the words
        heap.changeComparator((x, y) -> x.word.compareTo(y.word));

        //Array that will be send
        Tuple sorted[] = new Tuple[heap.size()];
